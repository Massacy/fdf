...
/*   Updated: 2023/07/26 02:51:15 by imasayos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calc_rotate(t_pos *pos, t_vars *vars)
{
	...
	x_old -= (vars->x_center * vars->base_zoom) * vars->zoom;
	y_old -= (vars->y_center * vars->base_zoom) * vars->zoom;
	pos->x1_v = x_old * cos(rad) - y_old * sin(rad);
	pos->y1_v = x_old * sin(rad) + y_old * cos(rad);
	pos->x1_v += (vars->x_center * vars->base_zoom) * vars->zoom;
	pos->y1_v += (vars->y_center * vars->base_zoom) * vars->zoom;
	x_old = pos->x2_v;
	y_old = pos->y2_v;
	x_old -= (vars->x_center * vars->base_zoom) * vars->zoom;
	y_old -= (vars->y_center * vars->base_zoom) * vars->zoom;
	...
	pos->x2_v += (vars->x_center * vars->base_zoom) * vars->zoom;
